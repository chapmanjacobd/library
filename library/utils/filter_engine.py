...
def filter_items_by_criteria(args, items):
    ...
    if getattr(args, "time_created", []):
        ...
        items = [d for d in items if d.get("time_created", 0) > 0 and args.time_created(d["time_created"])]  # type: ignore
    if getattr(args, "time_modified", []):
        items = [d if "time_modified" in d else file_utils.get_file_stats(d) for d in items]
        items = [d for d in items if d.get("time_modified", 0) > 0 and args.time_modified(d["time_modified"])]  # type: ignore
