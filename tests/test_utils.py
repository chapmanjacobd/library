...
def test_col_naturaldate():
    assert utils.col_naturaldate([{"t": 0, "t1": 1}], "t") == [{"t": None, "t1": 1}]
    assert utils.col_naturaldate([{"t": 0, "t1": 86399}], "t1") == [{"t": 0, "t1": "Jan 01 1970"}]
...
def test_clean_path():
    assert utils.clean_path(b"_/_test/-t") == p("_/_Test/t")
    ...
    assert utils.clean_path(b"/_test/t") == p("/_Test/t")
    assert utils.clean_path(b"/_test/t-") == p("/_Test/t")
    assert utils.clean_path(b"/test/\xff\xfeH") == p("/Test/xffxfeH")
